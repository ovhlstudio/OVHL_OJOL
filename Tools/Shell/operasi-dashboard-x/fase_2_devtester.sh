...
echo "🚀 FASE 2: Dev UI Tester Module"

# Create DevUITester module
mkdir -p Source/Core/Server/Modules/DevUITester
cat > Source/Core/Server/Modules/DevUITester/manifest.lua << 'EOF'
return {
    name = "DevUITester",
    version = "1.0.0", 
...
cat > Source/Core/Server/Modules/DevUITester/Handler.lua << 'EOF'
--!strict
local DevUITester = {}
...
# Create Client-side Dev UI Tester
mkdir -p Source/Core/Client/Modules/DevUITester
cat > Source/Core/Client/Modules/DevUITester/Main.lua << 'EOF'
--!strict
...
# Update ClientBootstrapper to load DevUITester
cat > Source/Core/Client/ClientBootstrapper.lua << 'EOF'
--!strict
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local Core = ReplicatedStorage:WaitForChild("Core")
local UIManager = require(Core.Client.Services.UIManager)
...
echo "✅ FASE 2 SELESAI: Dev UI Tester Module created"
echo "📝 TEST INSTRUCTION: Play test - harus ada button 'SHOW DEV TOOLS' untuk test components"
echo "🚀 LANJUT KE FASE 3: ./Tools/operasi-dashboard-x/fase_3_adminpanel.sh"