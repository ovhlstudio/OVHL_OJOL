#!/bin/bash
# ---
# 🚀 OVHL "RESET & REBORN" SAKTI v1.4 (FIXED Manifest Naming + SOP Logging)
# Nama Operasi: UPGRADE OVHL OS - ENTERPRISE (Fresh Install)
# Tujuan: 1. Bumi Hangus isi /Source/. 2. Rebuild struktur OS v2.1 dari NOL. 3. Pastikan ClientBootstrapper v2.2 (FIXED) terinstall!
# PERBAIKAN: Semua manifest.client.lua diubah menjadi manifest.client (tanpa .lua extension)
# ---

set -e # Keluar kalo ada error

echo "🚀 Memulai Operasi Bumi Hangus & Reborn OS (v1.4 - FIXED Manifest)..."
echo "--------------------------------------------------------"
...
# Core Client Bootstrapper v2.2 (FIXED Manifest Check + SOP Logging)
# INI VERSI FINAL DARI OTAK CLIENT!
cat <<'EOF' > Source/Core/Client/ClientBootstrapper.lua
--!strict
--[[
	@project OVHL_OJOL
	@file ClientBootstrapper.lua (v2.2 - FINALIZED + SOP Logging v1.0)
	@author OmniverseHighland + AI Co-Dev System
	@version 2.2.0

	@description
	"OTAK" Sisi Client v2.2. FIX bug manifest check (IsA -> pcall require).
	Bootstrapper ini 100% otomatis, plug-and-play, dan zero-touch.
]]
...
function ClientBootstrapper:Start()
	...
	for _, item in ipairs(ModulesPath:GetChildren()) do
		if item:IsA("Folder") then
			...
			local manifestScript = moduleFolder:FindFirstChild("manifest.client")

			if manifestScript then -- Cukup cek filenya ADA atau TIDAK
				...
				print(MONITOR_PREFIX .. ("[ClientBootstrapper] [FOLDER_NO_MANIFEST] [DEBUG] Folder '%s' tidak punya manifest.client, diabaikan (mungkin Service?)."):format(moduleFolder.Name))
	...
	for _, moduleInfo in ipairs(modulesToLoad) do
		...
		if manifest.autoInit == true then
			...
			-- Cari file .lua nya
			local entryScript = folder:FindFirstChild(entryName .. ".lua") or folder:FindFirstChild(entryName) -- Coba tanpa .lua juga
			...
			if entryScript and entryScript:IsA("ModuleScript") then
				...
				if success and typeof(module) == "table" and typeof(module.Init) == "function" then
					...
					if not initSuccess then
						print(MONITOR_PREFIX .. ("      ㄴ[ClientBootstrapper] [MODULE_INIT_FAIL] [ERROR] Gagal Init() modul '%s': %s"):format(manifest.name, initError))
						totalRusakEntry += 1
					else
				...
				print(MONITOR_PREFIX .. ("[ClientBootstrapper] [MODULE_LOAD_FAIL] [ERROR] Gagal meload modul RUSAK (entry file '%s.lua' tidak ditemukan): %s"):format(entryName, manifest.name))
				totalRusakEntry += 1
			end
		else
	...
	print(OS_PREFIX .. ("      ㄴ Rusak karena Manifes    : %d"):format(totalRusakManifest))
	print(OS_PREFIX .. ("      ㄴ Rusak karena Entry/Init : %d"):format(totalRusakEntry))
	print(OS_PREFIX .. "-------------------------------------------------------------")
...
# PlayerDataController (Manifest + Main) -- INI YANG KRITIS BUAT DEPENDENCY
cat <<'EOF' > Source/Core/Client/Modules/PlayerDataController/manifest.client
--!strict
-- Manifest untuk PlayerDataController
return {
...
# MainHUD (Manifest + Main) -- Butuh PlayerDataController dari DI
cat <<'EOF' > Source/Core/Client/Modules/MainHUD/manifest.client
--!strict
-- Manifest untuk MainHUD
return {
...
# DebugProtoActive A (Manifest + Main)
cat <<'EOF' > Source/Core/Client/Modules/ModPrototypeA/manifest.client
--!strict
-- Manifest ModPrototypeA (Aktif)
...
EOF
# DebugProtoActive B (Manifest + Main)
cat <<'EOF' > Source/Core/Client/Modules/ModPrototypeB/manifest.client
--!strict
-- Manifest ModPrototypeB (Aktif)
...
# DebugProtoDisabled C (Manifest Only)
cat <<'EOF' > Source/Core/Client/Modules/ModPrototypeC/manifest.client
--!strict
-- Manifest ModPrototypeC (Nonaktif)
return { name = "ModPrototypeC", autoInit = false, loadOrder = 200, entry = "Main" }
EOF
# (Tidak perlu Main.lua karena autoInit=false)

# DebugProtoBroken D (Manifest Only, No Main.lua)
cat <<'EOF' > Source/Core/Client/Modules/ModPrototypeD/manifest.client
--!strict
-- Manifest ModPrototypeD (Rusak)
...
echo "✅ OPERASI BUMI HANGUS & REBORN (v1.4 - FIXED) SELESAI!"
echo "Core OS Enterprise v2.1 (FIXED) + Modul Client terpasang."
echo "PERBAIKAN: Semua manifest.client.lua diubah menjadi manifest.client"
